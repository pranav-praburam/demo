...
function Box1() {
  return <Text>Quitting Cigarettes is very difficult and is statiscally one of the hardest addictions to kill. 1 in 5 Americans die of 
...
function Box2() {
  return <Text>Vaping although new, does not mean healthy. Although our knowledge on the long-term effects of vaping are unclear, we know that the nicotine levels
...
function Box3() {
  return <Text>Although it's socially acceptable at times, alcohol is not joke. In fact, it is the only drug which can kill heavy users if they try to quit
...
function Box4() {
  return <Text>Gambling, also known as the silent killer. Although it poses no adverse physical health effects, mentally it is a very taxing habit.
